...
public interface ProjectRepository extends JpaRepository<Project, UUID> {
    ...
    @Query("""
        ...
        WHERE LOWER(p.name) LIKE LOWER(CONCAT(:name, '%'))
          AND (p.user.id = :userId OR p.id IN (SELECT pm.project.id FROM ProjectMember pm WHERE pm.user.id = :userId))
        """)
    List<Project> findAllAccessibleByNamePrefix(@Param("name") String name, @Param("userId") UUID userId);
